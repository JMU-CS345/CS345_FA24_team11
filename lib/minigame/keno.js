...
function drawKeno() {
  ...
  if (isGameStarted) {
    ...
    text("Game.player.currency: $" + Game.player.currency, width / 2, height / 2 + 330);
  } else {
    fill(0);
    textSize(24);
  ...
  // Display Game.player.currency in the top-right corner
  fill(0);
  textSize(20);
  textAlign(CENTER);
  text(`Money: $${Game.player.currency}`, width - 100, 100);
...
function calculateMoney() {
  ...
  if (matches === 1) {
    Game.player.currency = betAmount; // 1x multiplier
  } else if (matches === 2) {
    Game.player.currency = betAmount * 3; // 3x multiplier
  } else if (matches === 3) {
    Game.player.currency = betAmount * 10; // 10x multiplier
  } else {
    Game.player.currency = 0; // No Game.player.currency
  }

  Game.player.currency += Game.player.currency - betAmount; // Deduct bet, add Game.player.currency
}
