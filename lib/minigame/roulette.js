...
function drawRouletteControls() {
  ...
  drawButton("Red", centerX - 160, baseY, 80, 40, color(220, 38, 38));
  drawButton("Black", centerX - 60, baseY, 80, 40, color(0));
  drawButton("Num.", centerX + 40, baseY, 80, 40, color(75, 85, 99));
...
function drawRouletteInfo() {
  ...
  text(`$${Game.player.currency}`, 35, 40);
...
function placeRouletteBet(type, value = null) {
  if (rouletteBetAmount > Game.player.currency) {
...
function spinRouletteWheel() {
  ...
  Game.player.currency -= rouletteBetAmount;
...
function determineRouletteResult() {
  ...
  if (betType === "number" && betValue === winningNumber) {
    Game.player.currency += rouletteBetAmount * 35;
    ...
    Game.player.currency += rouletteBetAmount * 2;
    resultMessage = `Winning number: ${winningNumber} (${winningColor}). You won $${
      rouletteBetAmount * 2
    }!`;
    winnerMessage = "You WIN!";
