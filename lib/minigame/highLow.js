...
function drawHighLowGame() {
  ...
  textAlign(LEFT);
  text("Money: $" + Game.player.currency, 20, 40);
...
function setupButtons() {
  ...
  betInput.input(() => {
    ...
    if (value > 0 && value <= Game.player.currency) {
      betAmount = value;
    } else {
...
function lockBet() {
  if (betAmount > Game.player.currency) {
...
function makeGuess(choice) {
  ...
  if (
    ...
    Game.player.currency -= winnings + betAmount;
    winnings = 0;
